...
**Автоматически:** push в ветку `pep8` → GitHub Actions тестирует, собирает Docker и деплоит на VPS.

**Вручную на сервере** (редко):

```bash
cd ~/uni_edu_platform
git fetch origin pep8 && git checkout pep8 && git pull --ff-only origin pep8
./scripts/deploy.sh
```

### Подтянуть изменения из STerrible (на компьютере)

```bash
cd /path/to/uni_edu_platform
git remote add upstream https://github.com/STerrible/uni_edu_platform.git   # один раз
git fetch upstream
git checkout pep8
git merge upstream/pep8
git push origin pep8    # → автодеплой
```
...
| Событие | Действие |
|---------|----------|
| Pull Request в `pep8` | Тесты (`pytest`) |
| Push в `pep8` | Тесты → сборка Docker → деплой на VPS |
| `workflow_dispatch` | То же, что push в `pep8` |
...
После push в `pep8`: вкладка **Actions** в репозитории. Продакшен: https://edu-platform.thenetbase.com/docs
